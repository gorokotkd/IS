...
public class InterfazRatings extends JDialog {
			...
			okButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					...
					if (isNumeric(valor)) {
							...
							//Gestor.getBd().ratingsDevolKeys().contains(numero)
							if (Intermediario.getIntermediario().ratingsDevolKeys().contains(numero)){
							ArrayList<Tupla<Integer,Double>> listaaux = Intermediario.getIntermediario().getRatingsPorId(numero);
							Vector<ListPeliNota> items1 = new Vector<ListPeliNota>();
	...
	private JList getList_1() {
		if (list == null) {
			...
			ArrayList<Integer> lista = Intermediario.getIntermediario().ratingsDevolKeys();
